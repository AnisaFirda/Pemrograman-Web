<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="Stylesheet" href="Asset/css/bootstrap.min.css">
    <title>Document</title>
</head>
<body>
    <div class="container">
        <div class="row">
            <div class="col-md-12" style="background-color: #B3C8CF; text-align: center; color:white; position: fixed;">
              <header>
                <h1>Selamat Datang di Website Saya</h1>
              </header>
            </div>
            <div class="col-md-3" style="background-color: #F1EEDC; color: #Black; padding: 15px;"><br><br>
                <h3>Anisa Firda Salsabila</h3>
                <h3><?php session_start(); echo $_SESSION ['nim']?></h3>
                <li><a href="halaman1.php">Beranda</a></li>
                <li><a href="BiodataModifBeranda.php">Biodata</a></li>
                <li><a href="JadwalKuliah.php">Jadwal kuliah</a></li>
                <li><a href="logout.php">Logout</a></li>
                <search>
                    <form>
                      <input name="fsrch" id="fsrch" placeholder="Serch Information" style="margin: 10px; ">
                    </form>
                </search>
            </div>
            <div class="col-md-9" style="background-color: white; text-align: justify">
              <div class="container">
                <h2 style="text-align: center;">Jadwal Kuliah Saya</h2>
                <div class="table-responsive">
                  <table class="table table-bordered">
                    <thead>
                      <tr>
                        <th>No</th>
                        <th>Kode MK</th>
                        <th>Nama Mata Kuliah</th>
                        <th>SKS</th>
                        <th>T/P</th>
                        <th>STS</th>
                        <th>Kelp</th>
                        <th>Hari,Jam</th>
                        <th>Ruang</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td>1</td>
                        <td>A12.66601</td>
                        <td>Perencaaan Sumber Daya Perusahaan</td>
                        <td>3</td>
                        <td>T</td>
                        <td>B</td>
                        <td>A12.6407</td>
                        <td>Senin, 18.30-21.00</td>
                        <td>H.3.1</td>
                      </tr>
                      <tr>
                        <td>2</td>
                        <td>A12.66403</td>
                        <td>Pemrograman Berorientasi Obyek</td>
                        <td>4</td>
                        <td>TP</td>
                        <td>B</td>
                        <td>A12.6402</td>
                        <td>Selasa, 07.00-08.40</td>
                        <td>D.2.K</td>
                      </tr>
                      <tr>
                        <td>3</td>
                        <td>A12.66404</td>
                        <td>Pemrograman Web</td>
                        <td>4</td>
                        <td>TP</td>
                        <td>B</td>
                        <td>A12.6404</td>
                        <td>Rabu, 08.40-10.20</td>
                        <td>H.4.8</td>
                      </tr>
                      <tr>
                        <td>4</td>
                        <td>A12.66403</td>
                        <td>Analisis Dan Perancangan SIstem Informasi</td>
                        <td>4</td>
                        <td>T</td>
                        <td>B</td>
                        <td>A12.6405</td>
                        <td>Rabu, 12.30-14.10</td>
                        <td>h.5.10</td>
                      </tr>
                      <tr>
                        <td>5</td>
                        <td>u201701</td>
                        <td>Dasar Kewirausahaan</td>
                        <td>2</td>
                        <td>T</td>
                        <td>B</td>
                        <td>A12.6604</td>
                        <td>Rabu, 14.10-15.50</td>
                        <td>Kulino</td>
                      </tr>
                      <tr>
                        <td>3</td>
                        <td>A12.66404</td>
                        <td>Pemrograman Web</td>
                        <td>4</td>
                        <td>TP</td>
                        <td>B</td>
                        <td>A12.6404</td>
                        <td>Kamis, 10.20-12.00</td>
                        <td>D.2.B</td>
                      </tr>
                      <tr>
                        <td>5</td>
                        <td>A12.66403</td>
                        <td>Pemrograman Berorientasi Obyek</td>
                        <td>4</td>
                        <td>TP</td>
                        <td>B</td>
                        <td>A12.6402</td>
                        <td>Kamis, 12.30.14.10</td>
                        <td>h.4.4</td>
                      </tr>
                      <tr>
                        <td>6</td>
                        <td>A12.66401</td>
                        <td>Analisis Dan Perancangan SIstem Informasi</td>
                        <td>4</td>
                        <td>T</td>
                        <td>B</td>
                        <td>A12.6405</td>
                        <td>Jumat, 07.00-08.40</td>
                        <td>h.3.9</td>
                      </tr>
                      <tr>
                          <td>7</td>
                          <td>A12.66603</td>
                          <td>Perencaanaan Strategis Sistem Informasi</td>
                          <td>3</td>
                          <td>T</td>
                          <td>B</td>
                          <td>A12.6402</td>
                          <td>Jumat, 07.30-12.00</td>
                          <td>h.3.10</td>
                        </tr>
                        <tr>
                            <td>8</td>
                            <td>A12.6660</td>
                            <td>Menejemen Rantai Pasok</td>
                            <td>3</td>
                            <td>T</td>
                            <td>B</td>
                            <td>A12.6401</td>
                            <td>Jumat, 12.30-15.00</td>
                            <td>h.4.8</td>
                          </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </div>
            <div class="col-md-12" style="background-color: #B3C8CF; text-align: center;color:white; padding: 15px 0;">
                <footer>
                    <p>Copyright &copy; 2024. All rights reserved.</p>
                </footer>
            </div>
       </div>
    </div>
</body>
</html>